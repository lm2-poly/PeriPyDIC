...
class elastic_material_dic():
    ...
    def __init__(self, PD_deck, PD_problem, t):
        ...
        self.exp_e = np.zeros( (int(self.Exp_Num_Nodes), int(self.Exp_Num_Nodes) ) ) 
        self.exp_M = np.zeros( (int(self.Exp_Num_Nodes), int(self.Exp_Num_Nodes) ) )
        self.exp_T = np.zeros( (int(self.Exp_Num_Nodes), int(self.Exp_Num_Nodes) ) )
    ...
    def compute_ext_state(self, PD_deck, PD_problem, t):        
        ...
        for x_i in range(0, self.Exp_Num_Nodes):
            if not(x_i - 1 < 0) :
                self.exp_e[x_i, x_i-1] = np.absolute(PD_problem.exp_displacement[t][x_i-1] - PD_problem.exp_displacement[t][x_i]) - np.absolute(PD_problem.exp_init_positions[x_i-1] - PD_problem.exp_init_positions[x_i])
    ...
    def compute_M(self, PD_problem, t):
        ...
        for x_i in range(0, self.Exp_Num_Nodes):
            y_i = (PD_problem.exp_displacement[t][x_i]+PD_problem.exp_init_positions[x_i])
            if not(x_i - 1 < 0) :
                y_i_m1 = (PD_problem.exp_displacement[t][x_i-1]+PD_problem.exp_init_positions[x_i-1])
                self.exp_M[x_i, x_i-1] = ( y_i_m1 - y_i ) / np.absolute(y_i_m1 - y_i)
            if not(x_i +1 > self.Exp_Num_Nodes-1):
                y_i_p1 = (PD_problem.exp_displacement[t][x_i+1]+PD_problem.exp_init_positions[x_i+1])
                self.exp_M[x_i, x_i+1] = ( y_i_p1 - y_i ) / np.absolute(y_i_p1 - y_i)
    ...
    def weighted_function(self, PD_problem, PD_deck, x, x_i):
        ...
        for x_i in range(0, self.Exp_Num_Nodes):
            if not(x_i - 1 < 0) :
                result += Influence_Function *  Delta_V * (PD_problem.exp_init_positions[x_i-1]-PD_problem.exp_init_positions[x_i])**2
            if not(x_i +1 > self.Exp_Num_Nodes-1):
                result += Influence_Function * Delta_V * (PD_problem.exp_init_positions[x_i+1]-PD_problem.exp_init_positions[x_i])**2
        return result
    ...
    def compute_T(self, PD_deck, PD_problem):
        ...
        for x_i in range(0, self.Exp_Num_Nodes):
            for e_i in range(0, len(self.exp_e[x_i])):
                self.exp_T[x_i, e_i] = self.exp_M[x_i, e_i]*(w/self.weighted_function(PD_problem, PD_deck, PD_problem.exp_init_positions[x_i], x_i))*self.Modulus*self.exp_e[x_i, e_i]

    def compute_Ts(self, PD_deck):
        for x_i in range(0, self.Exp_Num_Nodes):
            if not(x_i - 1 < 0) :
                self.exp_Ts[x_i] += self.exp_T[x_i, x_i-1] - self.exp_T[x_i-1, x_i]
            if not(x_i +1 > self.Exp_Num_Nodes-1):
                self.exp_Ts[x_i] += self.exp_T[x_i, x_i+1] - self.exp_T[x_i+1, x_i]
    ...
    def compute_exp_strain_energy(self, PD_problem, t):
        for x_i in range(0, self.Exp_Num_Nodes):
            self.exp_W[x_i] = self.exp_Ts[x_i] * PD_problem.exp_displacement[t][x_i]
