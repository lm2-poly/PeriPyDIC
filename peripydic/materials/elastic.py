    ...
    def compute_dilatation_slice(self, deck, data_solver, y, start, end):
        for i in range(start, end):
            ...
            for p in index_x_family:
                ...
                if deck.dim == 2:
                    self.dilatation[i] += (2. / self.Weighted_Volume[i]) * self.factor2d * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * linalgebra.norm(X) * self.e[i,n] * self.Volume_Correction[i,n] * deck.geometry.volumes[p]
        ...
        for i in range(start, end):
            ...
            for p in index_x_family:
                ...
                if deck.dim == 1:
                    ...
                    ## Scalar force state
                    self.t = alpha * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * self.e[i,n]
                ...
                if deck.dim == 2:
                    ...
                    e_d = self.e[i,n] - e_s
                    # Scalar force states
                    t_s = (2. * self.factor2d * alpha_s - (3. - 2. * self.factor2d) * alpha_d) * functions.damage(deck,data_solver,i, p) *functions.w(data_solver, X, deck.influence_function) * e_s / 3.
                    t_d = alpha_d * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * e_d
                    self.t = t_s + t_d
                ...
                if deck.dim == 3:
                    ...
                    e_d = self.e[i,n] - e_s
                    # Scalar force states
                    t_s = alpha_s * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * e_s
                    t_d = alpha_d * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * e_d
                    self.t = t_s + t_d
        ...
        for i in range(start, end):
            ...
            for p in index_x_family:
                ...
                if deck.dim == 1:
                    ...
                    self.strain_energy[i] += 0.5 * alpha * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * self.e[i,n]**2 * self.Volume_Correction[i,n] * deck.geometry.volumes[p]
                ...
                if deck.dim >= 2:
                    ...
                    e_d = self.e[i,n] - e_s
                    # Strain energy density
                    self.strain_energy[i] += 0.5 * functions.damage(deck,data_solver,i, p) * functions.w(data_solver, X, deck.influence_function) * (alpha_s * e_s**2 + alpha_d * e_d**2) * self.Volume_Correction[i,n] * deck.geometry.volumes[p]
                n += 1
